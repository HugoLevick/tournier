...
function getInviteHtml(invite) {
  ...
  <div class='noti-invite' style="font-size: 2rem">
    <div>
      Incoming invite from ${invite.fromTeam.captain.username} to <a href='/tournament?term=${invite.fromTeam.tourney.slug}' style='color: var(--primary); text-decoration:underline'>${tourneyTitle}</a>
    </div>
...
function getWarningHtml(warning) {
  let message = warning.message.replaceAll(user.username + "'s", 'your');
  if (warning.json) {
